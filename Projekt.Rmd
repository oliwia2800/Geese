...
OPISAĆ DOKŁADNIE KOLUMNY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Celem projektu jest analiza danych dotyczących udzielania kredytów hipotecznych i zidentyfikowanie, które czynniki mają wpływ na to, czy kredytobiorca regularnie spłaca kredyt, czy nie.\
...
\## Braki w danych W początkowym etapie analizy danych przystąpiono do identyfikacji braków danych.
...
Analizę opisową rozpoczęto od ponownego sprawdzenia struktury danych, aby zdecydować jakie styatystyki opisowe zostaną wykorzystane w kolejnych krokach. Następnie przekształcono dane, uważane przez nas za numeryczne, ale traktowane przez program jako tekstowe na typ numeric. Po tej operacji, zmienne te będą mogły być wykorzystywane w analizach opisowych.\

Następnie zdefiniowano funkcję do obliczania statystyk dla wybranych zmiennych jakimi będą:
wysokość kredytu oraz wysokość dochodów kredytobiorców. Zmienne te zostaną pogrupowane według zmiennej TARGET, aby sprawdzić różnice pomiędzy osobami spłacającymi kredyt oraz tymi, którzy posiadają problem ze spłatą. \
