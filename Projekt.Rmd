...
Z wykresu można wywnioskować, że najliczniejsza grupa niespłacająca kredytu należy do poziomu edukacji "secondary/secondary special", najmniej liczna natomiast do "lower secondary". Wykres potwierdza fakt, że większość osób z bazy danych spłaca swoje zobowiązania. \## Analiza opisowa

## Wnioskowanie (testy statystyczne)

Wniskowanie statystyczne rozpoczęto od ponownego sprawdzenia struktury danych, aby zdecydować jakie testy statystyczne zostaną wykonane dla danych ilościowych. Następnie przekształcono dane, uważane przez nas za numeryczne, ale traktowane przez program jako tekstowe za tekstowe na typ numeric. Po tej operacji, zmienne te będą mogły być wykorzystywane w analizach statystycznych.\

Następnie zdefiniowano funkcję do obliczania statystyk dla określonych kolumn. Statystykami obliczanymi w funkcji są:

-   średnia arytmetyczna wartości w kolumnie;
...
-   połowa zakresu międzykwartylowego;
...
```
Stworzono jeden zbiór wyników wykorzystując do tego funkcję, która połączyła wszystkie statystyki z listy "statistics_i" w jeden obiekt. 
...
```
Podsumowując wszystkie obliczone statystyki dla zmiennych, można zauważyć następujące ogólne tendencje - większość zmiennych w zestawie danych ma rozkład, który jest albo skondensowany wokół jednej wartości (np. TARGET, REGION_RATING_CLIENT), albo rozproszony w dużym zakresie (np. AMT_INCOME_TOTAL, AMT_CREDIT). Średnia pokazująca centralną tendencję zmiennej, czyli jej przeciętną wartość,  w przypadku zmiennych z dużymi wartościami skrajnymi (np. AMT_INCOME_TOTAL, AMT_CREDIT) średnia jest znacznie wyższa niż mediana. /
Moda wskazująca najczęściej występującą wartość w zbiorze danych w przypadku zmiennej TARGET  wynosi 0, co sugeruje, że większość danych to obserwacje z wartością 0. Taka sytuacja powatarza się w zestawieniu aż czterokrotnie./
